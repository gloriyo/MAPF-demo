...
class Figure:
    def __init__(self, my_map, starts, goals):
        ...
        for i in range(len(self.goals)):
            ...
            y = starts[i][0] # these are (y,x) coords... from the instance files
            x = starts[i][1]

            flipped_y = len(self.my_map[0]) -1 - starts[i][0]
            flipped_x = len(self.my_map) -1 - starts[i][1]

            self.agents[i] = Circle((x, flipped_y), 0.3, facecolor=Colors[i % len(Colors)],
                                    edgecolor='black')
            self.agents[i].original_face_color = Colors[i % len(Colors)]
            self.patches.append(self.agents[i])
            self.agent_names[i] = self.ax.text(x, flipped_y + 0.5, name)
        ...
        # for i in range(len(self.goals)):
        #     self.ax.add_patch(self.agents[i])
    ...
    def save(self, file_name):
        self.fig.savefig(file_name, dpi=200)
